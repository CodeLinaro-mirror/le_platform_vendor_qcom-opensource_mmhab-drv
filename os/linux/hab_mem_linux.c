...
void habmem_defer_unimp_sent(struct export_desc *export)
{
	...
	struct pages_list *pglist;
	struct import_priv_info *priv_info;

	dmabuf = (struct dma_buf *)export->kva;
	priv_info = (struct import_priv_info *)dmabuf->priv;
	pglist = priv_info->pglist;
...
static struct dma_buf *habmem_get_dma_buf_from_uva(unsigned long address,
		uint32_t page_count, struct physical_channel *pchan)
{
	struct page **pages = NULL;
	...
	struct import_priv_info *priv_info = NULL;

	priv_info = kzalloc(sizeof(struct import_priv_info), GFP_KERNEL);
	if (priv_info == NULL)
		return ERR_PTR(-ENOMEM);
	...
	/* priv should include pglist & vmid_local */
	priv_info->pglist = pglist;
	priv_info->vmid_local = pchan->vmid_local;
	exp_info.priv = priv_info;
		...
		uint32_t *data_size)
{
	...
	if (IS_ERR_OR_NULL(dmabuf) || metadata == NULL || data_size == NULL)
		return -EINVAL;
	...
	if (dmabuf->ops != &dma_buf_ops) {
		...
		for_each_sg(sg_table->sgl, s, sg_table->nents, i) {
			...
			if (j == 0) {
				...
				metadata->pfns.region[j-1].space = (int32_t)space;
			...
			metadata->pfns.region[j].size = spage_size - (uint32_t)page_offset;
			if (metadata->pfns.region[j].size >= (uint32_t)page_count) {
				metadata->pfns.region[j].size = (uint32_t)page_count;
				metadata->pfns.region[j].space = 0;
				break;
			}

			page_count -= (int)metadata->pfns.region[j].size;
			pre_page = nth_page(page, metadata->pfns.region[j].size - 1U);
			page_offset = 0;
			j++;
		}
		...
		for (i = 1; i < page_count; i++) {
			if ((page_to_pfn(pages[i]) - 1) ==
				...
				metadata->pfns.region[j].size = (uint32_t)region_size;
				metadata->pfns.region[j].space = (int32_t)space;
				j++;
				region_size = 1;
			}
		}
	...
	*data_size = (uint32_t)sizeof(struct compressed_pfns) +
		(uint32_t)sizeof(struct region) * (uint32_t)metadata->pfns.nregions;
		...
		int *export_id)
{
	...
	if (flags & HABMM_EXP_MEM_TYPE_LOOPBACK)
		...
		sizebytes = (uint32_t)sizeof(*exp_super) +
				(uint32_t)sizeof(struct compressed_pfns) +
				page_count * (uint32_t)sizeof(struct region);

	pr_debug("exp_desc %zu, comp_metadata %zu, region %zu, page_count %u\n",
	...
	metadata = (struct mem_share_metadata *)&export->payload[0];
		...
		int *export_id)
{
	...
	if ((HABMM_EXP_MEM_TYPE_DMA & flags) != 0U)
		...
		dmabuf = habmem_get_dma_buf_from_uva(address, page_count, vchan->pchan);
	else
		...
		int *export_id)
{
	...
	if ((HABMM_EXPIMP_FLAGS_DMABUF & flags) != 0U) {
		...
		/*
		 * priv_info is only needed for the kva path where we create a
		 * new dma-buf and must store pglist alongside it.  DMABUF/FD
		 * paths wrap an existing dma-buf and do not use priv_info.
		 */
		priv_info = kzalloc(sizeof(struct import_priv_info), GFP_KERNEL);
		if (!priv_info) {
			ret = -ENOMEM;
		...
		/* priv should include pglist & vmid_local */
		priv_info->pglist = pglist;
		priv_info->vmid_local = vchan->pchan->vmid_local;
		exp_info.priv = priv_info;
	...
	enum dma_data_direction direction)
{
	...
	struct import_priv_info *priv_info = dmabuf->priv;
	struct pages_list *pglist = priv_info->pglist;
	struct sg_table *sgt;
	struct scatterlist *sg;
	int i;
	int ret = 0;
	struct page **pages = pglist->pages;
	int vmid_local = priv_info->vmid_local;
	...
	/* TODO: VFIO_DEV_DT_NAME check may not needed. Need more time to do test future */
	if ((vmid_local == 0) ||
		(strstr(dev_name(attachment->dev), VFIO_DEV_DT_NAME) != NULL)) {
...
static struct dma_buf *habmem_get_lb_dmabuf(struct export_desc *export)
{
	struct dma_buf *dmabuf = NULL;
	...
	metadata = (struct mem_share_metadata *)export->payload;
	 ...
	 * TODO: support PVM import loopback memory, need VMID to identify pchan
	 */
	...
	uint32_t userflags)
{
	...
	if (exp_super->is_loopback) {
		dmabuf = habmem_get_lb_dmabuf(export);
	} else {
