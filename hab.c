...
void hab_ctx_free(struct kref *ref)
{
	...
	for (exp_super = hab_rb_min(&ctx->imp_whse, struct export_desc_super, node);
		...
		if (ret == 0) {
			/*
			 * even if imp_hyp_unmap return success, it is still an unexpected
			 * scenario: HAB client exits/crashes/gets killed before unimport
			 * all imported memory.
			 */
			pr_debug("unimp msg sent for exp id %u on %s\n",
				export->export_id, export->pchan->name);
			...
			if ((ret != 0) && (ret != -ENODEV))
				...
				pr_debug("unimp msg send ret %d, vcid %x, expid %u, pchan rmt closed %d\n",
					ret, export->vcid_local, export->export_id,
					export->pchan->otherend_closed);
		} else if (ret == -EBUSY) {
...
void hab_send_close_msg(struct virtual_channel *vchan)
{
	...
	if ((vchan != NULL) && (vchan->otherend_closed == 0)) {
		...
		HAB_HEADER_SET_ID(header, vchan->otherend_id);
		HAB_HEADER_SET_SESSION_ID(header, vchan->session_id);
		(void)physical_channel_send(vchan->pchan, &header, NULL);
	}
}
...
void hab_send_unimport_msg(struct virtual_channel *vchan, uint32_t exp_id)
{
	...
	if (vchan != NULL) {
		...
		HAB_HEADER_SET_ID(header, vchan->otherend_id);
		HAB_HEADER_SET_SESSION_ID(header, vchan->session_id);
		(void)physical_channel_send(vchan->pchan, &header, &exp_id);
	}
}
				...
				int vmid_local, int vmid_remote, int is_be)
{
	...
	(void)snprintf(pchan_name, MAX_VMID_NAME_SIZE, "vm%d-", vmid);
	(void)strlcat(pchan_name, mmid_device->name, MAX_VMID_NAME_SIZE);
